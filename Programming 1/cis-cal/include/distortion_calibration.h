...
namespace cis {
    ...
    template<typename T>
    Eigen::Matrix<T,Eigen::Dynamic,Eigen::Dynamic> distortion_function(const CalReadings<T> &readings, const std::vector<PointCloud<T>> &ret) {
        ...
        Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> F_mat = Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>::Zero(num_rows, 216);
        int counter = 0;
        for (size_t i = 0; i < num_frames; i++) {
            ...
            for (size_t j = 0; j < C.size(); j++) {
                ...
                Point u = scaleToBox(C.at(j), min(readings.em_marker_calobj()), max(readings.em_marker_calobj()));
        ...
        return F_mat.jacobiSvd(Eigen::ComputeFullU | Eigen::ComputeFullV).solve(
                P_mat);
    }
}
